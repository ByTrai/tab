...
export function parseRoadmap(markdown) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (
      index += 1;
      index < lines.length && !/^#{2,3} /.test(lines[index]);
      index += 1
    )
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--apply") options.apply = true;
      ...
      options.repo = requiredValue(argv, ++index, argument);
    else if (argument === "--roadmap")
      options.roadmap = requiredValue(argv, ++index, argument);
    else if (argument === "--card")
      options.cards.add(requiredValue(argv, ++index, argument));
    else if (argument === "--assign") {
      const [owner, login, ...extra] = requiredValue(
        argv,
        ++index,
        argument,
      ).split("=");
      if (!owner || !login || extra.length)
        throw new Error("--assign must use owner=github-login.");
...
async function main() {
  ...
  if (options.help) {
    console.log(
      "Usage: node scripts/create-roadmap-issues.mjs [--apply] [--repo owner/name] [--card T1.1] [--include-done] [--assign tommy=login]\n\nDry-run is the default. --apply creates missing issues and detects existing cards by a stable body marker.",
    );
    return;
  }
  const roadmapPath =
    options.roadmap ?? new URL("../roadmap.md", import.meta.url);
  ...
  if (unknownIds.length)
    throw new Error(`Unknown roadmap card(s): ${unknownIds.join(", ")}`);
  const cards = parsedCards.filter(
    (card) =>
      (options.includeDone || card.status !== "done") &&
      (!options.cards.size || options.cards.has(card.id)),
  );
  if (!options.apply) {
    console.log(
      JSON.stringify(
        cards.map(({ id, name, owner, status }) => ({
          id,
          title: `[${id}] ${name}`,
          owner,
          status,
    ...
    console.error(
      `Dry run: ${cards.length} issue(s) selected. Re-run with --apply to create missing issues.`,
    );
    return;
  }
  ...
  const existingMarkers = new Set(
    existing.flatMap((issue) =>
      [
        ...(issue.body ?? "").matchAll(
          /<!-- tabby-roadmap-card:(T\d+\.\d+) -->/g,
        ),
      ].map((match) => match[1]),
    ),
  );
  ...
  for (const card of cards) {
    ...
    const mappedAssignee = options.assignments.get(
      card.owner.replace(/^@/, ""),
    );
