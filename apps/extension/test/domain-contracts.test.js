...
test("migrates web schema v1 deterministically without mutating the fixture", () => {
  ...
  assert.deepEqual(migrated.workspaces[0], {
  ...
  assert.equal(
    serializeWorkspaceExport(migrated),
    serializeWorkspaceExport(migrateWorkspaceExport(snapshot)),
  );
});
...
test("rejects malicious or inconsistent exports before canonicalization", () => {
  ...
  const oversized = {
    ...fixture,
    items: Array(CONTRACT_LIMITS.items + 1).fill(null),
  };
  assert.throws(() => migrateWorkspaceExport(oversized), /supported limit/);
  assert.throws(
    () => migrateWorkspaceExport({ ...fixture, schemaVersion: 99 }),
    /Unsupported/,
  );
});

test("keeps current capture journals and rejects oversized or non-UTC legacy data", async () => {
  const { migrateCaptureState } =
    await import("../packages/workspace-contracts/index.js");
  const v1 = JSON.parse(
    await readFile(
      new URL("./fixtures/extension-journal-v1.json", import.meta.url),
      "utf8",
    ),
  );
  const v2 = JSON.parse(
    await readFile(
      new URL("./fixtures/extension-journal-v2.json", import.meta.url),
      "utf8",
    ),
  );
  ...
  assert.throws(
    () =>
      migrateCaptureState({
        ...v1,
        operations: Array(CONTRACT_LIMITS.captureOperations + 1).fill({
          id: "x",
        }),
      }),
    /collection limits/,
