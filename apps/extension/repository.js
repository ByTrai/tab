...
export function readCompatibleState(value) {
  if (value === undefined) return structuredClone(EMPTY_STATE);
  return migrateCaptureState(value);
}
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        request.error ?? new Error("Could not open Tabby's local database."),
      );
  });
}
...
export class IndexedDbRepository {
  async read() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        try {
          resolve(readCompatibleState(request.result));
        } catch (error) {
          reject(error);
        }
      };
      request.onerror = () =>
        reject(
          request.error ?? new Error("Could not read Tabby's local database."),
        );
  ...
  async mutate(change) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        try {
          store.put(change(readCompatibleState(request.result)), STATE_KEY);
        } catch {
          transaction.abort();
        }
      };
      ...
      transaction.onerror = () => {
        database.close();
        reject(
          transaction.error ?? new Error("Could not persist the capture."),
        );
      };
      transaction.onabort = () => {
        database.close();
        reject(
          transaction.error ??
            new Error("The capture transaction was cancelled."),
        );
      };
    });
  }
  ...
  async operation(id) {
    return (await this.read()).operations.find(
  ...
  async commitCapture(operation, items) {
    await this.mutate((state) =>
      state.operations.some(({ id }) => id === operation.id)
        ? state
        : {
            ...state,
  ...
  async updateOperation(operation) {
    await this.mutate((state) => ({
      ...
      operations: state.operations.map((entry) =>
        entry.id === operation.id ? operation : entry,
      ),
    }));
  }
