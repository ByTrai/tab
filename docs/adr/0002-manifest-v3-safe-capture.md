...
Shared entity vocabulary, URL rules, and the non-destructive journal version 1 to version 2 migration are governed by [ADR 0004](./0004-shared-workspace-contracts.md). The extension imports that dependency-free package from within its own root so loading it unpacked still requires no build step.
